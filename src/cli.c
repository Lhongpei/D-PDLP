...
void save_solver_summary(const cupdlpx_result_t *result, const char *output_dir,
  ...
  if (result->presolve_time > 0.0) {
    ...
    //     result->presolve_stats.nnz_removed_trivial); fprintf(outfile, "NNZ
    ...
    //     result->presolve_stats.nnz_removed_parallel_rows); fprintf(outfile,
    //     "NNZ Removed Parallel Cols: %d\n",
    //     result->presolve_stats.nnz_removed_parallel_cols);

    //     fprintf(outfile, "Presolve Time Init (sec): %e\n",
    //     result->presolve_stats.time_init); fprintf(outfile, "Presolve Time
    //     Run (sec): %e\n", result->presolve_stats.time_presolve);
    //     fprintf(outfile, "Presolve Time Fast (sec): %e\n",
    //     result->presolve_stats.time_fast_reductions); fprintf(outfile,
    //     "Presolve Time Medium (sec): %e\n",
    //     result->presolve_stats.time_medium_reductions); fprintf(outfile,
    //     "Presolve Time Primal Proppagation (sec): %e\n",
    //     result->presolve_stats.time_primal_propagation); fprintf(outfile,
    //     "Presolve Time Parallel Rows (sec): %e\n",
    //     result->presolve_stats.time_parallel_rows); fprintf(outfile,
    //     "Presolve Time Parallel Cols (sec): %e\n",
    //     result->presolve_stats.time_parallel_cols); fprintf(outfile,
    //     "Postsolve Time (sec): %e\n", result->presolve_stats.time_postsolve);
    // }
  }
...
int run_d_pdlpx(int argc, char *argv[]) {
      ...
      if (sscanf(optarg, "%d,%d", &r, &c) == 2) {
        if (r > 0 && c > 0) {
          if (r * c != world_size) {
            if (rank_global == 0) {
              ...
              fprintf(stderr,
                      "==============================================\n");
              fprintf(stderr,
                      "Command line input : --grid_size %s (Total: %d)\n",
                      optarg, r * c);
              fprintf(stderr, "MPI Runtime size   : -n %d\n", world_size);
              fprintf(stderr, "Reason             : r * c must equal the "
          ...
          if (rank_global == 0)
            fprintf(
                stderr,
                "Error: Grid dimensions must be positive integers. Got: %s\n",
                optarg);
          MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
      } else {
        if (rank_global == 0)
          fprintf(stderr, "Error: Invalid grid_size format. Use --grid_size "
                          "r,c (e.g., --grid_size 2,2)\n");
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
      }
      break;
    }
    case 2002: // --partition_method
    {
      if (strcmp(optarg, "uniform") == 0) {
        params.partition_method = UNIFORM_PARTITION;
                 ...
                 strcmp(optarg, "nnz_balance") == 0) {
        params.partition_method = NNZ_BALANCE_PARTITION;
      } else {
        if (rank_global == 0)
          fprintf(stderr,
                  "Error: partition_method must be 'uniform' or 'nnz'\n");
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
      }
    } break;
  ...
  if (instance_name == NULL) {
    if (rank_global == 0)
      fprintf(stderr,
              "Error: Could not extract instance name from filename.\n");
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    return 1;
  }
