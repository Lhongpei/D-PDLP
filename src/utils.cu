...
void pdhg_final_log(const cupdlpx_result_t *result,
  ...
  //            stats->n_rows_original, stats->n_cols_original,
  //            stats->nnz_original);
  //     printf("  Reduced            : %d rows, %d cols, %d nnz\n",
  //            stats->n_rows_reduced, stats->n_cols_reduced,
  //            stats->nnz_reduced);
  ...
  //     stats->nnz_removed_primal_propagation); printf("  Parallel Rows      :
  //     %d\n", stats->nnz_removed_parallel_rows); printf("  Parallel Cols :
  //     %d\n", stats->nnz_removed_parallel_cols);

  //     printf("  [Timing]\n");
  //     printf("  Total Presolve     : %.3g sec\n",
  //     stats->presolve_total_time); printf("  Init               : %.3g
  //     sec\n", stats->ps_time_init); printf("  Fast               : %.3g
  //     sec\n", stats->ps_time_fast); printf("  Medium             : %.3g
  //     sec\n", stats->ps_time_medium); printf("  Primal Propagation : %.3g
  //     sec\n", stats->ps_time_primal_propagation); printf("  Parallel Rows :
  //     %.3g sec\n", stats->ps_time_parallel_rows); printf("  Parallel Cols :
  //     %.3g sec\n", stats->ps_time_parallel_cols); printf("  Postsolve : %.3g
...
void compute_dual_feas_polish_residual(pdhg_solver_state_t *state,
  ...
  double base_dual_objective;
  CUBLAS_CHECK(cublasDdot(state->blas_handle, state->num_variables,
                          state->dual_slack, 1, ori_state->pdhg_primal_solution,
