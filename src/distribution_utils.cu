...
void csr_extract_submatrix(int n_total, int m_total, const int *A_row_ptr,
                           const int *A_col_ind, const double *A_val,
                           int row_start, int row_end, int col_start,
                           int col_end, int **sub_row_ptr, int **sub_col_ind,
                           double **sub_val, int *sub_nnz) {
...
lp_problem_t *partition_lp_problem(const lp_problem_t *global_lp,
  ...
  csr_extract_submatrix(
      n_total, m_total, global_lp->constraint_matrix_row_pointers,
      global_lp->constraint_matrix_col_indices,
      global_lp->constraint_matrix_values, m_start, m_end, n_start, n_end,
      &loc->constraint_matrix_row_pointers, &loc->constraint_matrix_col_indices,
      &loc->constraint_matrix_values, &loc->constraint_matrix_num_nonzeros);

  loc->objective_vector =
      copy_slice(global_lp->objective_vector, n_start, loc->num_variables);
  loc->variable_lower_bound =
      copy_slice(global_lp->variable_lower_bound, n_start, loc->num_variables);
  loc->variable_upper_bound =
      copy_slice(global_lp->variable_upper_bound, n_start, loc->num_variables);

  loc->constraint_lower_bound = copy_slice(global_lp->constraint_lower_bound,
                                           m_start, loc->num_constraints);
  loc->constraint_upper_bound = copy_slice(global_lp->constraint_upper_bound,
                                           m_start, loc->num_constraints);

  if (global_lp->primal_start) {
    loc->primal_start =
        copy_slice(global_lp->primal_start, n_start, loc->num_variables);
  }
  if (global_lp->dual_start) {
    loc->dual_start =
        copy_slice(global_lp->dual_start, m_start, loc->num_constraints);
  }
...
rescale_info_t *partition_rescale_info(rescale_info_t *global_info,
  ...
  rescale_info_t *loc_info =
      (rescale_info_t *)calloc(1, sizeof(rescale_info_t));

  int n_start, m_start;
  loc_info->scaled_problem = partition_lp_problem(
      global_info->scaled_problem, grid, method, &n_start, &m_start);

  lp_problem_t *loc_lp = loc_info->scaled_problem;

  loc_info->var_rescale =
      copy_slice(global_info->var_rescale, n_start, loc_lp->num_variables);

  loc_info->con_rescale =
      copy_slice(global_info->con_rescale, m_start, loc_lp->num_constraints);
...
size_t get_lp_problem_size(const lp_problem_t *lp) {
  ...
  if (lp->primal_start)
    size += sizeof(double) * lp->num_variables;
  if (lp->dual_start)
    size += sizeof(double) * lp->num_constraints;
...
#define S_ARR(arr, count, type)                                                \
  ...
  S_ARR(lp->constraint_matrix_col_indices, lp->constraint_matrix_num_nonzeros,
        int);
  S_ARR(lp->constraint_matrix_values, lp->constraint_matrix_num_nonzeros,
        double);
  ...
  if (has_primal)
    S_ARR(lp->primal_start, lp->num_variables, double);
  if (has_dual)
    S_ARR(lp->dual_start, lp->num_constraints, double);
...
#define D_ARR(dest, count, type)                                               \
  ...
  D_ARR(lp->constraint_matrix_col_indices, lp->constraint_matrix_num_nonzeros,
        int);
  D_ARR(lp->constraint_matrix_values, lp->constraint_matrix_num_nonzeros,
        double);
  ...
  if (has_primal)
    D_ARR(lp->primal_start, lp->num_variables, double);
  if (has_dual)
    D_ARR(lp->dual_start, lp->num_constraints, double);
...
void big_recv_bytes(void **buffer_ptr, size_t *size_ptr, int source,
  ...
  MPI_Recv(&total_len, 1, MPI_UNSIGNED_LONG_LONG, source, 0, comm,
           MPI_STATUS_IGNORE);
  ...
  *buffer_ptr = malloc((size_t)total_len);
  ...
  while (offset < total_len) {
    size_t remaining = total_len - offset;
    int current_chunk = (remaining > CHUNK_SIZE) ? CHUNK_SIZE : (int)remaining;
    MPI_Recv(buf + offset, current_chunk, MPI_BYTE, source, 1, comm,
             MPI_STATUS_IGNORE);
    offset += current_chunk;
  }
}
...
big_request_t big_isend_bytes(const void *buffer, size_t size, int dest,
  ...
  *p_len = size;
  ...
  while (offset < size) {
    ...
    int current_chunk = (remaining > CHUNK_SIZE) ? CHUNK_SIZE : (int)remaining;
    MPI_Isend(buf + offset, current_chunk, MPI_BYTE, dest, 1, comm,
              &breq.reqs[req_idx++]);
    offset += current_chunk;
  }
  return breq;
...
void distribute_data_bcast_then_partition(const lp_problem_t *working_problem,
  ...
  const lp_problem_t *current_working_problem = working_problem;
...

  {
    ...
    if (grid_context->rank_global == 0) {
      sz = get_lp_problem_size(current_working_problem);
      buf = (char *)malloc(sz);
      char *ptr_tmp = buf;
      serialize_lp_problem_to_ptr(current_working_problem, &ptr_tmp);
    }
  ...
  *out_local_resc = partition_rescale_info(
      current_rescale_info, grid_context, params->partition_method, NULL, NULL);
  *out_local_lp = partition_lp_problem(current_working_problem, grid_context,
  ...
  if (params->verbose && grid_context->rank_global == 0) {
    printf(
        "[Timer] Data Distribution (Bcast -> Partition) took %.3f seconds.\n",
        t_end - t_start);
  }
}
...
void distribute_data_partition_then_send(const lp_problem_t *working_problem,
  ...
  if (grid_context->rank_global == 0) {
    ...
    for (int r = 0; r < world_size; ++r) {
      ...
      lp_problem_t *sub_lp = partition_lp_problem(
          working_problem, &target_grid, params->partition_method, NULL, NULL);
      rescale_info_t *sub_rescale = partition_rescale_info(
          rescale_info, &target_grid, params->partition_method, NULL, NULL);
      ...
      size_t sz_lp = get_lp_problem_size(sub_lp);
      char *buf_lp = (char *)malloc(sz_lp);
      char *ptr_lp = buf_lp;
      serialize_lp_problem_to_ptr(sub_lp, &ptr_lp);
      ...
      *prev_len_resc = sz_resc;
      req_resc =
          big_isend_bytes(buf_resc, sz_resc, r, grid_context->comm_global);
...
double compute_global_norm(cublasHandle_t blas_handle, int m_local,
                           double *d_vec, MPI_Comm comm) {
  double local_norm_sq = 0.0;
  double global_norm_sq = 0.0;

  CUBLAS_CHECK(
      cublasDdot(blas_handle, m_local, d_vec, 1, d_vec, 1, &local_norm_sq));
...
double compute_global_dot(cublasHandle_t blas_handle, int m_local,
                          double *d_vec1, double *d_vec2, MPI_Comm comm) {
  double local_dot = 0.0;
  double global_dot = 0.0;

  CUBLAS_CHECK(
      cublasDdot(blas_handle, m_local, d_vec1, 1, d_vec2, 1, &local_dot));
...
double estimate_maximum_singular_value_distributed(
    ...
    const grid_context_t *grid_context, const cu_sparse_matrix_csr_t *A,
    const cu_sparse_matrix_csr_t *AT, int max_iterations, double tolerance) {
  int m = A->num_rows;
  int n = A->num_cols;
  ...
  CUSPARSE_CHECK(cusparseSpMV_bufferSize(
      sparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, matAT, vecEigen,
      &zero, vecDual, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG2, &bufferSizeAT));
  CUSPARSE_CHECK(cusparseSpMV_bufferSize(
      sparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, matA, vecDual,
      &zero, vecNextEigen, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG2, &bufferSizeA));
  CUDA_CHECK(cudaMalloc((void **)&dBufferAT, bufferSizeAT));
  CUDA_CHECK(cudaMalloc((void **)&dBufferA, bufferSizeA));

  double local_norm;
  CUBLAS_CHECK(
      cublasDnrm2_v2_64(blas_handle, m, eigenvector_d, 1, &local_norm));
  double local_norm_sq = local_norm * local_norm;
  double global_norm_sq = 0.0;

  MPI_Allreduce(&local_norm_sq, &global_norm_sq, 1, MPI_DOUBLE, MPI_SUM,
                grid_context->comm_col);
  ...
  for (int i = 0; i < max_iterations; ++i) {
    ...
    NCCL_CHECK(ncclAllReduce((const void *)dual_product_d,
                             (void *)dual_product_d, n, ncclDouble, ncclSum,
                             grid_context->nccl_col, 0));

    CUSPARSE_CHECK(cusparseSpMV(sparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                &one, matA, vecDual, &zero, vecNextEigen,
                                CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG2, dBufferA));

    NCCL_CHECK(ncclAllReduce((const void *)next_eigenvector_d,
                             (void *)next_eigenvector_d, m, ncclDouble, ncclSum,
                             grid_context->nccl_row, 0));

    double local_dot;
    CUBLAS_CHECK(cublasDdot(blas_handle, m, next_eigenvector_d, 1,
                            eigenvector_d, 1, &local_dot));

    MPI_Allreduce(&local_dot, &sigma_max_sq, 1, MPI_DOUBLE, MPI_SUM,
                  grid_context->comm_col);

    double neg_sigma_sq = -sigma_max_sq;
    CUBLAS_CHECK(cublasDaxpy(blas_handle, m, &neg_sigma_sq, eigenvector_d, 1,
    ...
    MPI_Allreduce(&local_res_sq, &global_res_sq, 1, MPI_DOUBLE, MPI_SUM,
                  grid_context->comm_col);
    ...
    CUBLAS_CHECK(cublasDaxpy(blas_handle, m, &sigma_max_sq, eigenvector_d, 1,
                             next_eigenvector_d, 1));

    double local_norm;
    CUBLAS_CHECK(
        cublasDnrm2_v2_64(blas_handle, m, next_eigenvector_d, 1, &local_norm));
    double local_norm_sq = local_norm * local_norm;
    double global_norm_sq = 0.0;
    MPI_Allreduce(&local_norm_sq, &global_norm_sq, 1, MPI_DOUBLE, MPI_SUM,
                  grid_context->comm_col);
...
void initialize_step_size_and_primal_weight_distributed(
  ...
  MPI_Allreduce(&local_nnz, &global_nnz, 1, MPI_LONG_LONG, MPI_SUM,
  ...
  if (global_nnz == 0) {
    ...
    double max_sv = estimate_maximum_singular_value_distributed(
        state->sparse_handle, state->blas_handle, state->grid_context,
        state->constraint_matrix, state->constraint_matrix_t,
...
void gather_distributed_vector(double *d_local_vec, int local_len,
  ...
  if (rank_check == 0) {
    ...
    CUDA_CHECK(cudaMemcpy(h_local, d_local_vec, local_len * sizeof(double),
    ...
    MPI_Gatherv(h_local, local_len, MPI_DOUBLE, h_global, counts, displs,
                MPI_DOUBLE, 0, comm_gather);
