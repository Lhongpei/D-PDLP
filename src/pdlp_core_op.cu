...
cupdlpx_result_t *
create_result_from_state(pdhg_solver_state_t *state,
...
void feasibility_polish(const pdhg_parameters_t *params,
  ...
  pdhg_solver_state_t *primal_state =
...
initialize_primal_feas_polish_state(const pdhg_solver_state_t *original_state) {
  pdhg_solver_state_t *primal_state =
      (pdhg_solver_state_t *)safe_malloc(sizeof(pdhg_solver_state_t));
...
#define ALLOC_AND_COPY_DEV(dest, src, bytes)                                   \
  ...
  ALLOC_AND_COPY_DEV(primal_state->current_primal_solution,
                     ...
                     num_var * sizeof(double));
  ALLOC_AND_COPY_DEV(primal_state->pdhg_primal_solution,
                     original_state->pdhg_primal_solution,
                     num_var * sizeof(double));
...
initialize_dual_feas_polish_state(const pdhg_solver_state_t *original_state) {
  pdhg_solver_state_t *dual_state =
      (pdhg_solver_state_t *)safe_malloc(sizeof(pdhg_solver_state_t));
...
#define ALLOC_ZERO(dest, bytes)                                                \
  ...
  ALLOC_AND_COPY_DEV(dual_state->current_dual_solution,
                     ...
                     num_cons * sizeof(double));
  ALLOC_AND_COPY_DEV(dual_state->pdhg_dual_solution,
                     original_state->pdhg_dual_solution,
                     num_cons * sizeof(double));
