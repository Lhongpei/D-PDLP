...
lp_problem_t *convert_pslp_to_cupdlpx(PresolvedProblem *reduced_prob) {
  ...
  lp_problem_t *cupdlpx_prob =
      (lp_problem_t *)safe_malloc(sizeof(lp_problem_t));
...
cupdlpx_presolve_info_t *pslp_presolve(const lp_problem_t *original_prob,
  ...
  if (original_prob->primal_start || original_prob->dual_start) {
    printf("Warning: Warm-starting is currently not supported when presolve is "
           "enabled.\n"
  ...
  cupdlpx_presolve_info_t *info = (cupdlpx_presolve_info_t *)safe_calloc(
      1, sizeof(cupdlpx_presolve_info_t));
  ...
  info->presolver = new_presolver(
      ...
      original_prob->num_constraints, original_prob->num_variables,
      original_prob->constraint_matrix_num_nonzeros,
      original_prob->constraint_lower_bound,
      original_prob->constraint_upper_bound,
      original_prob->variable_lower_bound, original_prob->variable_upper_bound,
      original_prob->objective_vector, info->settings);
  ...
  if (status & INFEASIBLE || status & UNBNDORINFEAS) {
    ...
    info->reduced_problem =
        convert_pslp_to_cupdlpx(info->presolver->reduced_prob);
  }
  return info;
}
...
create_result_from_presolve(const cupdlpx_presolve_info_t *info,
  ...
  // TODO: Verify if setting solution pointers to NULL affects Python/Julia
  // bindings.
  if (result->num_variables > 0) {
    result->primal_solution =
        (double *)safe_calloc(result->num_variables, sizeof(double));
    result->reduced_cost =
        (double *)safe_calloc(result->num_variables, sizeof(double));
  }
  if (result->num_constraints > 0) {
    result->dual_solution =
        (double *)safe_calloc(result->num_constraints, sizeof(double));
  }
  return result;
}

void pslp_postsolve(cupdlpx_presolve_info_t *info, cupdlpx_result_t *result,
                    const lp_problem_t *original_prob) {
  postsolve(info->presolver, result->primal_solution, result->dual_solution,
            result->reduced_cost, result->primal_objective_value);
  ...
  result->primal_solution =
      (double *)safe_malloc(original_prob->num_variables * sizeof(double));
  result->dual_solution =
      (double *)safe_malloc(original_prob->num_constraints * sizeof(double));
  result->reduced_cost =
      (double *)safe_malloc(original_prob->num_variables * sizeof(double));

  memcpy(result->primal_solution, info->presolver->sol->x,
         original_prob->num_variables * sizeof(double));
  memcpy(result->dual_solution, info->presolver->sol->y,
         original_prob->num_constraints * sizeof(double));
  memcpy(result->reduced_cost, info->presolver->sol->z,
         original_prob->num_variables * sizeof(double));
  // result->primal_objective_value = info->presolver->sol->obj; // This is a
  // bug in PSLP. We don't need to updated primal_objective_value since offset
  // has been updated during presolve. Therefore, the original problem and
  // reduced problem have the same objective value.
  result->presolve_time = info->presolve_time;
