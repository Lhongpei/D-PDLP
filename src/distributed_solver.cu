...
static void allreduce_obj_bound_norm(pdhg_solver_state_t *state,
                                     const pdhg_parameters_t *params);
cupdlpx_result_t *
create_result_from_state_distributed(pdhg_solver_state_t *state,
...
static lp_problem_t *permute_lp_problem(const pdhg_parameters_t *params,
                                        const lp_problem_t *original_problem,
                                        int **out_row_perm, int **out_col_perm);
static void repermute_solution(cupdlpx_result_t *result, int *row_perm,
                               int *col_perm);
...
cupdlpx_result_t *distributed_optimize(const pdhg_parameters_t *params,
  ...
  if (params->permute_method != NO_PERMUTATION) {
    ...
    if (grid_context.rank_global == 0) {
      permuted_problem =
          permute_lp_problem(params, original_problem, &row_perm, &col_perm);
    }

    result =
        distributed_optimize_core(&sub_params, permuted_problem, &grid_context);
    ...
    result =
        distributed_optimize_core(&sub_params, original_problem, &grid_context);
  }
  return result;
}
...
distributed_optimize_core(const pdhg_parameters_t *params,
  ...
  MPI_Bcast(&is_solved_during_presolve, 1, MPI_INT, 0,
            grid_context->comm_global);

  if (is_solved_during_presolve) {
    if (grid_context->rank_global == 0) {
      cupdlpx_result_t *result =
          create_result_from_presolve(presolve_info, original_problem);
      if (presolve_info)
  ...
  lp_problem_t *local_working_problem = NULL;
  rescale_info_t *local_rescale_info = NULL;

  // distribute_data_partition_then_send(
  //     working_problem, rescale_info, grid_context, params,
  //     &local_working_problem, &local_rescale_info
  // );
...
static void allreduce_obj_bound_norm(pdhg_solver_state_t *state,
  ...
  if (params->optimality_norm == NORM_TYPE_L_INF) {
    ...
    MPI_Allreduce(&local_val, &state->objective_vector_norm, 1, MPI_DOUBLE,
                  MPI_MAX, state->grid_context->comm_row);
  } else {
    double local_sq =
        state->objective_vector_norm * state->objective_vector_norm;
  ...
  if (params->optimality_norm == NORM_TYPE_L_INF) {
    ...
    MPI_Allreduce(&local_val, &state->constraint_bound_norm, 1, MPI_DOUBLE,
                  MPI_MAX, state->grid_context->comm_col);
  } else {
    double local_sq =
        state->constraint_bound_norm * state->constraint_bound_norm;
    double global_sq = 0.0;
    MPI_Allreduce(&local_sq, &global_sq, 1, MPI_DOUBLE, MPI_SUM,
...
create_result_from_state_distributed(pdhg_solver_state_t *state,
  ...
  NCCL_CHECK(ncclAllReduce((const void *)state->dual_product,
                           (void *)state->dual_product, state->num_variables,
                           ncclDouble, ncclSum, state->grid_context->nccl_col,
                           0));
  ...
  gather_distributed_vector(state->pdhg_primal_solution, state->num_variables,
                            state->grid_context->comm_col,
                            state->grid_context->comm_row, &global_primal);

  gather_distributed_vector(
      state->dual_slack, state->num_variables, state->grid_context->comm_col,
      state->grid_context->comm_row, &global_reduced_cost);

  gather_distributed_vector(state->pdhg_dual_solution, state->num_constraints,
                            state->grid_context->comm_row,
                            state->grid_context->comm_col, &global_dual);
  ...
  if (params->grid_size.decided) {
    ...
    if (product != world_size) {
      if (rank_global == 0) {
        ...
        fprintf(stderr,
                "Please adjust -n (mpirun) or --n_row_tiles/--n_col_tiles.\n");
        fprintf(stderr, "------------------------------------------------\n");
      }
    ...
    if (rank_global == 0) {
      get_best_grid_dims(original_problem->num_constraints,
                         original_problem->num_variables, world_size, &dims[0],
                         &dims[1]);
...
static void repermute_solution(cupdlpx_result_t *result, int *row_perm,
  ...
  permute_double_array(result->primal_solution, result->num_variables,
                       inv_col_perm);
  permute_double_array(result->dual_solution, result->num_constraints,
                       inv_row_perm);
  permute_double_array(result->reduced_cost, result->num_variables,
                       inv_col_perm);
  free(inv_col_perm);
  free(inv_row_perm);
}
