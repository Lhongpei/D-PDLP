...
void compute_residual_distributed(pdhg_solver_state_t *state,
  ...
  if (optimality_norm == NORM_TYPE_L_INF) {
    local_primal_res = get_vector_inf_norm(
  ...
  if (optimality_norm == NORM_TYPE_L_INF) {
    local_dual_res = get_vector_inf_norm(
    ...
    CUBLAS_CHECK(cublasDnrm2_v2_64(state->blas_handle, state->num_variables,
                                   state->dual_residual, 1, &local_dual_res));
  ...
  MPI_Allreduce(&local_dual_slack_sum, &global_dual_slack_sum, 1, MPI_DOUBLE,
                MPI_SUM, state->grid_context->comm_col);
...
void compute_infeasibility_information_distributed(pdhg_solver_state_t *state) {
  ...
  MPI_Allreduce(&local_primal_ray_inf_norm, &global_primal_ray_inf_norm, 1,
  ...
  MPI_Allreduce(&local_dual_ray_inf_norm, &dual_ray_inf_norm, 1, MPI_DOUBLE,
  ...
  MPI_Allreduce(&local_primal_ray_linear_objective,
                &state->primal_ray_linear_objective, 1, MPI_DOUBLE, MPI_SUM,
                state->grid_context->comm_row);
  ...
  MPI_Allreduce(&local_sum_primal_slack, &sum_primal_slack, 1, MPI_DOUBLE,
                MPI_SUM, state->grid_context->comm_col);
  MPI_Allreduce(&local_sum_dual_slack, &sum_dual_slack, 1, MPI_DOUBLE, MPI_SUM,
                state->grid_context->comm_row);
  ...
  MPI_Allreduce(&local_primal_slack_norm, &primal_slack_norm, 1, MPI_DOUBLE,
                MPI_MAX, state->grid_context->comm_row);
  MPI_Allreduce(&local_dual_slack_norm, &dual_slack_norm, 1, MPI_DOUBLE,
                MPI_MAX, state->grid_context->comm_col);
